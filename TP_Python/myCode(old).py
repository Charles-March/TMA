...
def compressionVideo(name,ext,quality,N,M):
    img0 = getGrayfromNumber(name,0,ext)
    img1 = getGrayfromNumber(name,1,ext)
    VecteurMouvement = calculVecteurDeplacement(img0,img1,eq)
    if(len(VecteurMouvement)==0):
        print("aucun vecteur de mouvement trouver entre "+getName(name,0)+" et "+getName(name,1))
        return ;
    print(VecteurMouvement)
...
compressionVideo("taxi_","bmp",1,16,16)
